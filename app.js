...
function renderGranth(){
  ...
  items.forEach(k=>{
    ...
    const rows=[...(k.rows||[])].reverse().map(r=>{
      ...
      return `<tr><td>${r.chakra}</td><td>${r.y ?? '-'}</td><td>${meta.ySelCode}</td><td>${meta.yHitCode}</td><td>${r.k ?? '-'}</td><td>${meta.kSelCode}</td><td>${meta.kHitCode}</td><td>${formatRoundInfoEntries(meta.capped)}</td><td>${formatRoundInfoEntries(meta.returned)}</td></tr>`;
    }).join('');
...
function advanceAfterLossSilent(side,rowEvents){
  for(let n=1;n<=9;n++){
...
function replayKumbhRowsWithCurrentSettings(kumbh){
  ...
  for(const row of rows){
    if(state.settings.keypadMode === 'combined'){
      ...
      if(y===0) advanceAfterLossSilent('Y',rowEvents); else { advanceAfterLossSilent('Y',rowEvents); resolveNumberSilent('Y', y,rowEvents); }
      if(k===0) advanceAfterLossSilent('K',rowEvents); else { advanceAfterLossSilent('K',rowEvents); resolveNumberSilent('K', k,rowEvents); }
