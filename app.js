...
function escapeCsvValue(value){ const str=String(value ?? ''); if(/[",\n]/.test(str)) return `"${str.replace(/"/g,'""')}"`; return str; }
function parseCsvLine(line){ const values=[]; let current=''; let inQuotes=false; for(let i=0;i<line.length;i++){ const ch=line[i]; if(ch==='"'){ if(inQuotes && line[i+1]==='"'){ current+='"'; i++; } else { inQuotes=!inQuotes; } } else if(ch===',' && !inQuotes){ values.push(current); current=''; } else { current+=ch; } } values.push(current); return values; }
function buildCsv(rows){ return rows.map(row=>row.map(escapeCsvValue).join(',')).join('\n'); }
async function saveTextFile({ suggestedName, content, mimeType='text/plain', description='Files', extension='.txt' }){ if(window.showSaveFilePicker){ try{ const handle=await window.showSaveFilePicker({ suggestedName, types:[{ description, accept:{ [mimeType]:[extension] } }] }); const writable=await handle.createWritable(); await writable.write(content); await writable.close(); return 'saved'; }catch(err){ if(err && err.name==='AbortError') return 'cancelled'; } } downloadFile(suggestedName,content,mimeType); return 'downloaded'; }
function ladderCsvContent(){ return buildCsv([['ladder','step','bet'], ...state.ladder.map((row,idx)=>['L1',`S${idx+1}`,row.bet])]); }
async function exportLadderCsv(){ syncFirstLadderFromInputs(); const content=ladderCsvContent(); const result=await saveTextFile({ suggestedName:'sopana-ladder.csv', content, mimeType:'text/csv', description:'CSV Files', extension:'.csv' }); if(result==='cancelled') return; showToast('SOPANA EXPORTED', result==='saved' ? 'Ladder CSV saved' : 'Ladder CSV downloaded'); }
async function exportDrishtiCsv(){ const content=buildCsv([['Side','Number','ActivationChakra','WinChakra','StepsToWin','PreviousLoss','WinningBet','NetProfitLoss','Status'], ...state.drishti.map(r=>[r.side,r.number,r.activationChakra,r.winChakra,r.steps,r.prevLoss,r.winBet,r.net,r.status])]); const result=await saveTextFile({ suggestedName:'drishti.csv', content, mimeType:'text/csv', description:'CSV Files', extension:'.csv' }); if(result==='cancelled') return; showToast('DRISHTI EXPORTED', result==='saved' ? 'CSV saved' : 'CSV downloaded'); }
function exportDrishtiPdf(){ const html=`<html><head><title>Drishti</title><style>body{font-family:Arial;padding:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #888;padding:6px;text-align:left}h1{font-size:20px}</style></head><body><h1>KUBERA WARHUNT V5 Pro - DRISHTI</h1>${q('drishtiTable').outerHTML}</body></html>`; const w=window.open('','_blank'); if(!w) return; w.document.write(html); w.document.close(); w.focus(); setTimeout(()=>w.print(), 250); }
...
function granthCsvContent(){ const rows=[['KumbhId','RowIndex','Y','K','Chakra','Exposure','ProfitLoss','LiveBankrollAfter']]; state.granth.forEach(k=>{ const kumbhRows=Array.isArray(k.rows) ? k.rows : []; if(!kumbhRows.length){ rows.push([k.id,'','','','','','','']); return; } kumbhRows.forEach((row,idx)=>{ rows.push([k.id,idx+1,row.y ?? '',row.k ?? '',row.chakra ?? '',row.exposure ?? '',row.profitLoss ?? '',row.liveBankroll ?? '']); }); }); return buildCsv(rows); }
async function exportGranthCsv(){ const content=granthCsvContent(); const result=await saveTextFile({ suggestedName:'granth-history.csv', content, mimeType:'text/csv', description:'CSV Files', extension:'.csv' }); if(result==='cancelled') return; showToast('GRANTH EXPORTED', result==='saved' ? 'History CSV saved' : 'History CSV downloaded'); }
function importGranthJson(e){ const file=e.target.files[0]; if(!file) return; file.text().then(text=>{ const name=String(file.name || '').toLowerCase(); if(name.endsWith('.csv')){ const lines=text.trim().split(/\r?\n/).slice(1).filter(Boolean); const grouped=new Map(); lines.forEach(line=>{ const [kumbhIdRaw,rowIndex,y,k,chakra,exposure,profitLoss,liveBankroll]=parseCsvLine(line); const kumbhId=Number(kumbhIdRaw); if(!Number.isFinite(kumbhId) || kumbhId<=0) return; if(!grouped.has(kumbhId)) grouped.set(kumbhId,[]); if(String(rowIndex).trim()==='' && String(y).trim()==='' && String(k).trim()==='') return; grouped.get(kumbhId).push({ rowIndex:Number(rowIndex)||0, row:{ y:y || null, k:k || null, chakra:Number(chakra)||0, exposure:Number(exposure)||0, profitLoss:Number(profitLoss)||0, liveBankroll:Number(liveBankroll)||0 } }); }); state.granth=[...grouped.entries()].sort((a,b)=>a[0]-b[0]).map(([id,rows],idx)=>({ id:idx+1, rows:rows.sort((a,b)=>a.rowIndex-b.rowIndex).map(item=>item.row) })); } else { state.granth=JSON.parse(text); } state.currentKumbhId=state.granth.at(-1)?.id||null; renderAll(); showToast('GRANTH LOADED','History imported'); }); e.target.value=''; }
function importLadderCsv(e){ const file=e.target.files[0]; if(!file) return; file.text().then(text=>{ const lines=text.trim().split(/\r?\n/).slice(1).filter(Boolean); let cumulative1=0, cumulative2=0; lines.forEach(line=>{ const [ladder,stepLabel,betRaw]=parseCsvLine(line); const idx=Math.max(0, Number(String(stepLabel).replace(/\D/g,''))-1); const bet=Number(betRaw)||0; if(String(ladder).trim().toUpperCase()==='L2'){ cumulative2 += bet; } else { cumulative1 += bet; state.ladder[idx] = { step:`S${idx+1}`, bet, winReturn:bet*9, netProfit:(bet*9)-cumulative1, ifLoseTotal:-cumulative1 }; } }); const hasRecordedRows = state.granth.some(k => Array.isArray(k.rows) && k.rows.length); if(hasRecordedRows) replayAllKumbhsWithCurrentSettings(); renderAll(); showToast('SOPANA LOADED','CSV ladder loaded'); }).finally(()=>{ e.target.value=''; }); }
...
function setupControls(){
  ...
  q('exportGranthBtn').addEventListener('click', exportGranthCsv); q('importGranthBtn').addEventListener('click', ()=>q('importGranthFile').click()); q('importGranthFile').addEventListener('change', importGranthJson);
