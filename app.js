...
// 🔥 BULLETPROOF MODAL LOGIC: Cleanly opens without binding inline events
function askModal({ title, text, okLabel='OK', cancelLabel='Cancel', okClass='warn' }){
  return new Promise(resolve=>{
    ...
    if(okBtn) {
        ...
        okBtn.className = okClass === 'warn' ? 'warn' : ''; // Reset classes cleanly
    }
...
// 🔥 GLOBAL MODAL CLICK DELEGATE: Captures taps directly and flawlessly
document.addEventListener('click', (e) => {
    const overlay = q('confirmOverlay');
    if (overlay && !overlay.classList.contains('hidden')) {
        const btnOk = e.target.closest('#confirmOkBtn');
        const btnCancel = e.target.closest('#confirmCancelBtn');
        
        if (btnOk) {
            e.preventDefault(); e.stopPropagation();
            closeClearKumbh(true);
        } else if (btnCancel) {
            e.preventDefault(); e.stopPropagation();
            closeClearKumbh(false);
...
function formatNextAhuti(side){ 
    ...
    for(let n=1;n<=9;n++){ 
        ...
        if(preview){ 
            ...
            groups.get(preview.bet).push(`[ ${n} ] (Lv${preview.stepLabel.replace('T', '')})`); 
        } 
    } 
    const parts=[...groups.entries()].sort((a,b)=>b[0]-a[0]).map(([bet,arr])=>`+ ${bet} ➔ ${arr.join(' & ')}`); 
...
function kumbhInsights(rows){
  ...
  for(const row of sortedRows){
    ...
    const meta = {
      ySelCode:'-', yHitCode:'-',
      kSelCode:'-', kHitCode:'-',
      capped:Array.isArray(row.cap) ? row.cap.map(c=>`[ ${c.replace(/\D/g, '')} ]`) : [],
      returned:Array.isArray(row.ret) ? row.ret.map(r=>`[ ${r.replace(/\D/g, '')} ]`) : []
    };
...
async function resolveNumber(side,num,notes,rowEvents){ 
  ...
  if(info.status==='C'){
    ...
    info.status='B'; info.ladder=2; info.step=1; info.pendingSecond=false; if(rowEvents) rowEvents.ret.push(`${side}${num}`); notes.push({title:'SUMMON REVIVED',text:`[ ${num} ] back in battle`,kind:'warn'}); return;
  }
  const mode = getAttackMode();
...
function readYantraSettings(){
  ...
  current.vaultBg = q('setVaultBg')?.value || 'bg-molten';
