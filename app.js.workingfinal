...
function kumbhInsights(rows){
  ...
  for(const row of sortedRows){
    ...
    const meta = { 
        ySelCode:'-', yHitCode:'-', kSelCode:'-', kHitCode:'-', 
        capped:Array.isArray(row.cap) ? row.cap : [], 
        returned:Array.isArray(row.ret) ? row.ret : [] 
    };
    ...
    if (window.showSaveFilePicker) {
        try {
            const handle = await window.showSaveFilePicker({
                ...
                types: [{ description: fileDesc, accept: { [mimeType]: [ext] } }]
            });
            ...
            console.warn('Path picker blocked, using direct download.', err);
        }
    }
...
async function doExportData(format) {
    ...
    if(format === 'csv') { 
        content = granthCsvContent(); type = 'text/csv'; ext = '.csv'; desc = 'CSV Files';
    } else if(format === 'xlsx') { 
        content = buildXlsxWorkbook(granthWorkbookSheets()); type = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'; ext = '.xlsx'; desc = 'Excel Files';
    } else { 
        content = JSON.stringify(exportPayload(), null, 2); type = 'application/json'; ext = '.json'; desc = 'JSON Files';
    }
...
function processDataImport(text) {
    ...
    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
        ...
        try {
            ...
            const firstRowText = grid.length ? grid[0].join(',').toLowerCase() : '';
            if (firstRowText.includes('kumbhid') || firstRowText.includes('chakra')) {
                dataRows = grid.slice(1);
            }
            
            dataRows.forEach(cols => {
                if (!cols || cols.length < 2) return;
                const rawKumbh = String(cols[0] || '').replace(/[^0-9]/g, '');
                const rawChakra = String(cols[1] || '').replace(/[^0-9]/g, '');
                
                const kumbhId = Number(rawKumbh) || 0; 
                const chakra = Number(rawChakra) || 0; 
                if (!kumbhId || !chakra) return; 
                
                if (!grouped.has(kumbhId)) grouped.set(kumbhId, { id: kumbhId, rows: [] }); 
                const target = grouped.get(kumbhId); 
                
                const strY = String(cols[2] || '').replace(/[^0-9]/g, '');
                const strK = String(cols[5] || '').replace(/[^0-9]/g, '');
                const valY = strY ? Number(strY) : '-';
                const valK = strK ? Number(strK) : '-';
                ...
                if(!target.rows.some(r => Number(r.chakra) === chakra)) {
                    target.rows.push({ 
                        ...
                        y: valY, 
                        k: valK, 
                        cap: cols[8] && cols[8] !== '-' ? cols[8].split('|').map(s=>s.trim()).filter(Boolean) : [], 
                        ret: cols[9] && cols[9] !== '-' ? cols[9].split('|').map(s=>s.trim()).filter(Boolean) : [], 
                        np: cols[10] && cols[10] !== '-' ? cols[10].split('|').map(s=>s.trim()).filter(Boolean) : [], 
                        ahuti: Number(String(cols[11]||'').replace(/[^0-9-]/g,'')) || 0, 
                        axyapatra: Number(String(cols[12]||'').replace(/[^0-9-]/g,'')) || 0 
                    }); 
                }
            });
