...
function renderGranth(){
  ...
  items.forEach(k=>{
    ...
    const rows=[...(k.rows||[])].reverse().map(r=>{
      ...
      return `<tr><td>${r.chakra}</td><td>${meta.ySelCode}</td><td>${meta.yHitCode}</td><td>${meta.kSelCode}</td><td>${meta.kHitCode}</td><td>${formatRoundInfoEntries(meta.capped)}</td><td>${formatRoundInfoEntries(meta.returned)}</td></tr>`;
    }).join('');
